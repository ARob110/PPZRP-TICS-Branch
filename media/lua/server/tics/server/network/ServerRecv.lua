...
RecvServer['ChatMessage'] = function(player, args)
    ...
    -- 2)  If it was a /faction message, also broadcast it as /low
    if args.type == 'faction' then
        -- shallow‑copy the original packet so we can tweak it
        local lowArgs = {}
        for k, v in pairs(args) do lowArgs[k] = v end

        lowArgs.type        = 'low'       -- treat as low chat
        lowArgs.disableVerb = args.disableVerb -- keep same verb flag
        lowArgs.textColor  = ChatMessage.MessageTypeSettings['low'].color

        -- **very important** : avoid an infinite “echo loop”
        lowArgs._echo = true            -- tag the packet

        ChatMessage.ProcessMessage(player, lowArgs, 'ChatMessage', false)
    end
end
...
RecvServer["SetBio"] = function(player, args)
    -- Accept nil (client sent `/bio clear`) or a string
    local raw = args and args.bioText        -- may be nil

    -- 1)  If nil or empty‑after‑trim => clear
    if raw == nil then
    ...
    local text = raw:match("^%s*(.-)%s*$")   -- trim

    if text:lower() == "clear" or #text == 0 then
        BioServer.Set(player, nil)
        ServerSend.ChatInfoMessage(player, "Bio cleared.")
        return
    elseif #text > 100 then
        ServerSend.ChatErrorMessage(player, nil, "Bio too long (max 100 chars).")
        return
    end
