...
function LanguageManager:getRandomMessage(message, language)
    if language == "AmericanSignLanguage" then
        return "[Signs something " .. self:getSignLanguageDescriptor(message) .. ".]"
    end

    -- default scramble logic
    local newMessage = ''
    for i = 1, #message do
...
function LanguageManager:getSignLanguageDescriptor(message)
    local length = #message
    if length < 20 then
        return "briefly"
    elseif length < 50 then
        return "with quick, fluid movements"
    elseif length < 100 then
        return "with intricate gestures"
    else
        return "in a series of detailed and expressive motions"
    end
end
