...
local function Lerp(start, target, progression)
    if type(start) ~= "number" or type(target) ~= "number" then
        return start or 0
    end
...
function ABubble:parseMessage(length)
    -- If length is not provided or not a number, use the length of the message.
    local messageLength = tonumber(length) or (#self.message or 0)
    if self.showPlayerName and self.playerName then
        local nameLength = #self.playerName + 1  -- trailing space
        messageLength = messageLength - nameLength
        if messageLength < 0 then
            messageLength = 0
        end
    end

    local parsedMessages = Parser.ParseTicsMessage(self.message or "", self.color, 20, messageLength)
    ...
    if self.showPlayerName then
        ...
        truncatedName = (#truncatedName <= (tonumber(length) or #self.message)) and truncatedName or truncatedName:sub(1, #truncatedName)
...
function ABubble:updateText(x, y)
    ...
    if self.voice then
        ...
        -- If the voice code isn't incrementing length properly,
        -- or length is 0, just show the full message
        if not length or length < 1 then
            length = self.fullMessageLength
        end
    else
        -- No voice, so display the entire message
        self.messageFinishedScrolling = true
        length = self.fullMessageLength
    end

    -- Ensure self.message is a valid string
