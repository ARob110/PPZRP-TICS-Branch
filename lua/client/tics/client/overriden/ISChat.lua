...
function ProcessStopSoundCommand(arguments)
...
local function ProcessTicsCommand(ticsCommand, message)
    ...
    elseif ticsCommand['name']== 'bio' then
        -- Get arguments (text after /bio )
        local arguments = GetArgumentsFromMessage(ticsCommand, message)

        if not arguments or arguments == "" then
            -- No arguments, show usage
            ISChat.sendInfoToCurrentTab("Usage: /bio <your bio text>  OR  /bio clear")
            return true -- Command was handled (showed usage)
        elseif arguments:lower() == "clear" then
            -- User wants to clear bio
            ClientSend.sendSetBio("clear") -- Send "clear" to the server
            return true -- Command was handled
        else
            -- User provided bio text
            -- Optional: Client-side length check (mirror server validation)
            local maxBioLength = 100 -- Match server limit
            if #arguments > maxBioLength then
                ISChat.sendErrorToCurrentTab("Bio text is too long (max " .. maxBioLength .. " characters).")
                return true -- Command recognized but failed validation
            end
            -- Send the bio text to the server
            ClientSend.sendSetBio(arguments)
            return true -- Command was handled
        end
...
function ISChat:onCommandEntered()
    ...
    if command and command ~= '' then
        ISChat.lastSentMessage = command
    end
    -- ******************************************
...
function ISChat.onPressUp(textEntryBox)
    -- Check ISChat scope first if necessary, but global should work
    local last = ISChat.lastSentMessage
    if last and #last > 0 then
        textEntryBox:setText(last)
        -- Optional: Move cursor to the end after setting text
        textEntryBox:setCursorPos(#last)
    end
end
...
local function highlightQuotedText(original, highlightColor, defaultColor)
    -- Build color tags for highlight vs. default
    ...
    -- This pattern captures anything between straight quotes "..."
    -- Then wraps that region with highlight color, returning to default color afterward.
    local pattern = "\"(.-)\""

    -- E.g. /me drank. "Woo!" becomes /me drank. <HIGHLIGHT>"Woo!"<DEFAULT>
    local replaced = original:gsub(pattern, function(capturedText)
        return highlightTag .. "\"" .. capturedText .. "\"" .. defaultTag
    end)
...
local function TICS_ApplySelfInjury(args)
    ...
    elseif injuryStr == "Bite" then
        bodyPart:setBitten(true, true) -- Corrected case from previous fix
    ...
    if injuryApplied then
        if ISChat and ISChat.sendInfoToCurrentTab then
            ISChat.sendInfoToCurrentTab("<RGB:1.0,0.5,0.5>" .. feedbackMsg) -- Use a reddish color
        else
            print("TICS ERROR: Could not send injury success feedback - ISChat.sendInfoToCurrentTab is nil")
        end

        -- Trigger health panel update safely
        bodyDamage:Update()
    ...
    elseif feedbackMsg then -- Handle cases like "cannot fracture"
        if ISChat and ISChat.sendInfoToCurrentTab then
            ISChat.sendInfoToCurrentTab("<RGB:1.0,1.0,0.5>" .. feedbackMsg) -- Yellowish for info/warning
        else
