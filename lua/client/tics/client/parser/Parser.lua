...
function Parser.ParseTicsMessage(message, defaultColor, wrapWords, maxBubbleLength)
    -- Step 1: Check for nil or empty input.
    if not message or message == "" then
        return { body = "", bubble = "", rawMessage = "" }
    end

    -- Step 2: Wrap FormatMessage in pcall to catch errors.
    local success, chatMessage, bubbleMessage, rawMessage = pcall(function()
        return FormatMessage(message, defaultColor, wrapWords, maxBubbleLength)
    end)
    if not success then
        print("Parser.ParseTicsMessage error during formatting:", chatMessage)
        -- Fallback: return the original message for all fields.
        return { body = message, bubble = message, rawMessage = message }
    end

    -- Step 3: Ensure returned values are strings.
    if type(chatMessage) ~= "string" then chatMessage = tostring(chatMessage) end
    if type(bubbleMessage) ~= "string" then bubbleMessage = tostring(bubbleMessage) end
    if type(rawMessage) ~= "string" then rawMessage = tostring(rawMessage) end

    -- Step 4: Return the parsed message table.
    return {
        ['body'] = chatMessage,
