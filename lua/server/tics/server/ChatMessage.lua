...
function ChatMessage.SaveBio(player, bioText)
    -- Input Validation (keep this)
    if not bioText then
        ServerSend.ChatErrorMessage(player, nil, "Bio text cannot be empty.")
        return
    end

    local modData = player:getModData()
    if not modData then
        print("TICS Error: Cannot set bio, ModData not found for " .. player:getUsername())
        ServerSend.ChatErrorMessage(player, nil, "Internal error: Could not access player data.")
        return
    end

    local onlineID = player:getOnlineID()
    local bioToSend = nil -- Will be nil if clearing

    -- Handle clearing the bio
    if bioText:lower() == "clear" then
        modData["PlayerFullBio"] = nil
        player:transmitModData()
        -- bioToSend remains nil
        ServerSend.ChatInfoMessage(player, "Bio cleared.")
        print("Player " .. player:getUsername() .. " cleared their bio.") -- Server log
    else
        -- Length Validation (keep this)
        local maxBioLength = 100
        if #bioText > maxBioLength then
            ServerSend.ChatErrorMessage(player, nil, "Bio text is too long (max " .. maxBioLength .. " characters).")
            return
        end

        -- Bad Word Check (Optional - keep if desired)
        --[[
        local foundBadWord = containsBadWord(bioText)
        if foundBadWord then
            ServerSend.ChatErrorMessage(player, nil, "Bio rejected due to inappropriate content: " .. foundBadWord)
            return
        end
        --]]

        -- Store full bio in ModData
        modData["PlayerFullBio"] = bioText
        player:transmitModData()
        bioToSend = bioText -- Set the bio to send to clients
        ServerSend.ChatInfoMessage(player, "Bio updated.")
        print("Player " .. player:getUsername() .. " updated their bio.") -- Server log
    end

    -- *** Broadcast the bio update to ALL clients ***
    local onlinePlayers = getOnlinePlayers()
    for i = 0, onlinePlayers:size() - 1 do
        local targetPlayer = onlinePlayers:get(i)
        -- Send the new command with the player's ID and the bio text (or nil if cleared)
        ServerSend.Command(targetPlayer, "UpdatePlayerBio", {
            onlineID = onlineID,
            bio = bioToSend
        })
    end
